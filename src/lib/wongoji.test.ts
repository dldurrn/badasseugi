...
describe('왕복 — 내장 문제 전체', () => {
  ...
  /**
   * 20단계를 뺀 모든 문장은 원고지 한 줄에 들어가야 합니다.
   * 학교 문제지가 문장 하나에 한 줄이고, 두 줄이 되면 확인 버튼이 아래로 밀립니다.
   *
   * 20단계만 예외입니다. 거기는 **두 문장을 이어 받아쓰는 것**이 목적이라
   * 칸에 맞추려고 줄이면 마지막 단계가 마지막 단계가 아니게 됩니다.
   */
  it('20단계를 뺀 문장은 모두 한 줄에 들어간다', () => {
    const over: string[] = [];
